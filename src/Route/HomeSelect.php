...
class HomeSelect  extends RouteBase {
	...
	private function getForm(): CustomForm {
		...
		$menu->setTitle(Utils::getText($this->getUserEntity()->getName(), "ADMIN_TOOLS_SELECT_HOME_TITLE"));
		$this->options = [];
		$this->optionsBis = [];
		foreach (MainAPI::getFactionHomes($this->getTargetFaction()) as $name => $home) {
			$this->options[] = $name . "(" . $home->toString() . ")";
			$this->optionsBis[] = $name;
		}
		if (count($this->options) != 0) {
			$menu->addDropdown(Utils::getText($this->getUserEntity()->getName(), "ADMIN_TOOLS_SELECT_HOME_INFORMATION"), $this->options);
			$this->menuActive = true;
		} else {
			$menu->addLabel(Utils::getText($this->getUserEntity()->getName(), "ADMIN_TOOLS_SELECT_HOME_ERROR"));
		}
		return $menu;
	}
}