...
class DeleteHome extends RouteBase {
			...
			return Utils::processMenu(RouterFactory::get(HomeSelect::SLUG), $player, [
				...
				function (string $factionName, string $homeName) use ($player) {
					...
					Utils::newMenuSendTask(new MenuSendTask(
						...
						function () use ($player, $factionName, $homeName) {
							(new FactionHomeDeleteEvent($player, $player->getName(), $factionName, true))->call();
							Utils::processMenu($this->getBackRoute(), $player, [Utils::getText($this->getUserEntity()->getName(), "ADMIN_TOOLS_DELETE_HOME_SUCCESS", ['homeName' => $homeName, "factionName" => $factionName])] );
						},
						function () use ($player) {
							Utils::processMenu($this->getBackRoute(), $player, [Utils::getText($this->getUserEntity()->getName(), "ERROR")]);
						}
					));
				},
				$this->getBackRoute()
	...
	private function getForm(string $message = "") : CustomForm {
		...
		$menu->addInput(Utils::getText($this->getUserEntity()->getName(), "ADMIN_TOOLS_INSTRUCTION"), Utils::getText($this->getUserEntity()->getName(), "ADMIN_TOOLS_DELETE_HOME_PLACEHOLDER"));
		return $menu;
	}
}